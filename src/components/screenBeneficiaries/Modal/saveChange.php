...
if ($mode === 'Editar') {
    ...
    $sql = "UPDATE beneficiaries SET name='$name', birth='$birth', tel='$tel', branch='$branch', personal_file='$personal_file', medical_file='$medical_file', active='$active' WHERE id=$id";
} else if ($mode === 'Añadir') {
    // Consulta para agregar un nuevo beneficiario
    $sql = "INSERT INTO beneficiaries (name, birth, tel, branch, personal_file, medical_file, active) VALUES ('$name', '$birth', '$tel', '$branch', '$personal_file', '$medical_file', '$active')";
} else {
